...
| Cloudflare / mothergod.dev | the project's domain and zone (operator-purchased) | owned web channel: project site, docs, blog; infrastructure (e.g. Telegram webhook worker); DNS | `MOTHERGOD_CLOUDFLARE_TOKEN` (zone-scoped, all zone permission groups; account-level Workers permissions may need an operator top-up) | 2026-08-20 |
