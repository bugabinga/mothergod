...
| Cloudflare (account) | workers.dev subdomain `mothergod`, created 2026-08-23 by the BDFL | Cloudflare refuses to deploy a Worker carrying a Durable Object until the account has one (API error 10063), so it exists to unblock PR #155 and nothing else. Nothing is served there: every worker sets `workers_dev = false` and answers on its own custom domain. Globally unique, so it is a name we hold | `CLOUDFLARE_API_TOKEN` (`PUT /accounts/:id/workers/subdomain`) | 2026-08-23 |
| Cloudflare Workers | `mothergod-telegram`, custom domain `bot.mothergod.dev` | the status bot's webhook (issue #5): authenticates Telegram's secret token, stores operator updates in KV namespace `mothergod-telegram-inbox`, runs the "typing..." indicator, and under it a draft of what the run is doing (`tg-draft`, #733), until `tg-send` reports the answer sent, wakes the BDFL via `workflow_dispatch`; deployed by `.github/workflows/deploy-telegram-worker.yml` | `CLOUDFLARE_API_TOKEN` (deploy + BDFL KV reads); worker secrets `WEBHOOK_SECRET` (from repo secret `TELEGRAM_WEBHOOK_SECRET`, also given to BDFL runs so `tg-send` can reach the worker) and `GITHUB_PAT` (from `TELEGRAM_GITHUB_PAT`, narrow PAT: actions + issues write, this repo only) | 2026-08-22 |
