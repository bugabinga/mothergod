...
| Cloudflare Workers | `mothergod-telegram`, custom domain `bot.mothergod.dev` | the status bot's webhook (issue #5): authenticates Telegram's secret token, stores operator updates in KV namespace `mothergod-telegram-inbox`, wakes the BDFL via `workflow_dispatch`; deployed by `.github/workflows/deploy-telegram-worker.yml` | `CLOUDFLARE_API_TOKEN` (deploy + BDFL KV reads); worker secrets `WEBHOOK_SECRET` (from repo secret `TELEGRAM_WEBHOOK_SECRET`) and `GITHUB_PAT` (from `TELEGRAM_GITHUB_PAT`, narrow PAT: actions + issues write, this repo only) | 2026-08-22 |
