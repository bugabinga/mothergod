...
use crate::literal::{ColumnExpertState, Context, Literal, MatchModelExpertState, PpmExpertState};
...
/// `research/JOURNAL.md` S1-P8's paired measurement, [`walk_tokens`]'s use
/// in [`ideal_cost_bits_matchmodel_expert_experiment`]: sums the same
/// flag/length/offset/slot costs [`CostSink`] does, so any delta between
/// `cost.baseline` and `cost.candidate` is attributable to the literal
/// model alone, and prices every literal byte twice through
/// [`Literal::ideal_cost_bits_matchmodel_expert_pair`] — the shipped
/// six-expert mix, and the same mix with a
/// [`crate::match_model::MatchModel`] prediction blended in as a genuinely
/// additive expert. Unlike [`PpmExpertCostSink`], this sink also owns
/// `data` directly: [`MatchModelExpertState::predict`] needs the raw byte
/// buffer and the current position, which [`TokenSink::literal`]'s own
/// signature does not carry (`Context` alone cannot derive this expert's
/// prediction, unlike every other expert in this lead line).
struct MatchModelCostSink<'a> {
    data: &'a [u8],
    match_state: &'a mut MatchModelExpertState,
    cost: PairedCost,
}

impl TokenSink for MatchModelCostSink<'_> {
    fn flag(&mut self, models: &mut Models, flag_table: usize, kind: usize) {
        self.cost
            .add_same(models.flag[flag_table].ideal_cost_bits(kind));
    }
    ...
    fn literal(&mut self, models: &mut Models, context: Context, byte: u8) {
        let predicted = self.match_state.predict(self.data, context.position);
        let (baseline, with_matchmodel) = models.literal.ideal_cost_bits_matchmodel_expert_pair(
            context,
            byte,
            predicted,
            self.match_state,
        );
        self.cost.add(baseline, with_matchmodel);
    }
...
/// `research/JOURNAL.md` S1-P8's before-wiring measurement: does blending a
/// hash-based match model in as a genuinely additive expert help, priced
/// through [`Literal::ideal_cost_bits_matchmodel_expert_pair`], the same
/// "measure before wiring" shape [`ideal_cost_bits_ppm_expert_experiment`]
/// and S1-P5's column expert each used for their own candidate expert. Not
/// reachable from [`encode`]/[`decode`]: no `Method`/`FORMAT_VERSION`
/// wiring, measurement only.
///
/// Returns `(baseline_bits, with_matchmodel_bits)`.
#[must_use]
pub fn ideal_cost_bits_matchmodel_expert_experiment(data: &[u8]) -> (f64, f64) {
    let tokens = lz::parse_optimal(data);
    let mut models = Models::new();
    let mut match_state = MatchModelExpertState::new();
    let mut sink = MatchModelCostSink {
        data,
...
mod tests {
    ...
    #[test]
    fn ideal_cost_bits_matchmodel_expert_experiment_is_zero_on_empty_input() {
        let (baseline, with_matchmodel) = ideal_cost_bits_matchmodel_expert_experiment(b"");
        assert!(baseline.abs() < 1e-9);
        assert!(with_matchmodel.abs() < 1e-9);
    }

    #[test]
    fn ideal_cost_bits_matchmodel_expert_experiment_stays_finite_and_positive() {
        // research/JOURNAL.md S1-P8: no accuracy claim here, just that the
        // paired walk runs to completion and both totals land somewhere
        // sane — the actual accept/reject verdict is a train/sealed
        // measurement recorded in the journal, not a unit test assertion.
        let data: &[u8] = include_bytes!("../research/imports/session-1/mothergod.rs");
        let (baseline, with_matchmodel) = ideal_cost_bits_matchmodel_expert_experiment(data);
        assert!(
            baseline.is_finite() && baseline > 0.0,
            "baseline={baseline}"
        );
    ...
    /// A fresh [`MatchModelCostSink`] over a fresh [`Models`]/
    /// [`MatchModelExpertState`] pair, for asserting each [`TokenSink`]
    /// method against a value computed independently from the same
    /// starting state.
    fn fresh_matchmodel_expert_sink() -> (Models, MatchModelExpertState) {
        (Models::new(), MatchModelExpertState::new())
    }

    #[test]
    fn matchmodel_expert_cost_sink_flag_adds_the_model_cost_to_both_totals() {
        let (mut expected_models, _) = fresh_matchmodel_expert_sink();
        let expected = expected_models.flag[0].ideal_cost_bits(FLAG_LITERAL);

        let (mut models, mut match_state) = fresh_matchmodel_expert_sink();
        let mut sink = MatchModelCostSink {
            data: b"",
            match_state: &mut match_state,
            cost: PairedCost::default(),
    ...
    #[test]
    fn matchmodel_expert_cost_sink_length_adds_the_bucketed_cost_to_both_totals() {
        let (mut expected_models, _) = fresh_matchmodel_expert_sink();
        let expected = ideal_cost_bucketed(&mut expected_models.length, 17);
    ...
    #[test]
    fn matchmodel_expert_cost_sink_offset_adds_the_bucketed_cost_to_both_totals() {
        let (mut expected_models, _) = fresh_matchmodel_expert_sink();
        let expected = ideal_cost_bucketed(&mut expected_models.offset, 123);
    ...
    #[test]
    fn matchmodel_expert_cost_sink_slot_adds_the_model_cost_to_both_totals() {
        let (mut expected_models, _) = fresh_matchmodel_expert_sink();
        let expected = expected_models.slot.ideal_cost_bits(0);
    ...
    #[test]
    fn matchmodel_expert_cost_sink_literal_adds_the_baseline_and_with_match_pair_separately() {
        // A repeated pattern long enough for the match model to lock onto
        // a real candidate partway through, so a sink that swaps or drops
        // one half of the pair is distinguishable from a correct one.
        let bytes: &[u8] = b"abcabcabcabcabcabcabcabc";

        let (mut expected_models, mut expected_match_state) = fresh_matchmodel_expert_sink();
        let (mut models, mut match_state) = fresh_matchmodel_expert_sink();
        let mut sink = MatchModelCostSink {
            data: bytes,
            match_state: &mut match_state,
        ...
        let mut context = Context::default();
        let mut expected_baseline_total = 0.0;
        let mut expected_with_match_total = 0.0;
        for (position, &byte) in bytes.iter().enumerate() {
            let predicted = expected_match_state.predict(bytes, position);
            let (baseline, with_match) = expected_models
                .literal
                .ideal_cost_bits_matchmodel_expert_pair(
                    context,
                    byte,
                    predicted,
                    &mut expected_match_state,
                );
            expected_baseline_total += baseline;
            expected_with_match_total += with_match;
            sink.literal(&mut models, context, byte);
            context = context.after_literal(byte);
        }

        assert!(
            (expected_baseline_total - expected_with_match_total).abs() > 1e-6,
            "baseline_total={expected_baseline_total} with_match_total={expected_with_match_total}, \
             test cannot tell the pair's two halves apart"
        );
        assert!((sink.cost.baseline - expected_baseline_total).abs() < 1e-6);
        assert!((sink.cost.candidate - expected_with_match_total).abs() < 1e-6);
    }
