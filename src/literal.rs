...
/// Number of buckets [`match_len_bucket`] sorts a [`crate::match_model::MatchModel`]
/// prediction's confirmed length into: no active candidate, a freshly
/// found one (length `0`, unconfirmed), then four widening length bands.
/// `research/JOURNAL.md` S1-P8's own hypothesis for this slice: a genuine,
/// long-confirmed match should earn a mixing weight a short or
/// just-guessed one never does, so match length needs to be part of *which
/// weight* [`MatchModelExpertState`] adapts, not folded into a single
/// scalar the way `research/JOURNAL.md` S2-R20's rejected matched-literal
/// expert was.
const MATCH_LEN_BUCKETS: usize = 6;

/// Sorts a [`crate::match_model::MatchModel::predict`] result into one of
/// [`MATCH_LEN_BUCKETS`] bands: `0` for "no candidate at all" (silent,
/// contributes zero mass — see [`Literal::mix_matchmodel`]), `1` for a
/// just-found, unconfirmed candidate, then widening bands so a handful of
/// confirmed hits and a long, well-established run land in different
/// weight slots.
fn match_len_bucket(predicted: Option<(u8, u32)>) -> usize {
    match predicted {
        None => 0,
        Some((_, 0)) => 1,
        Some((_, len)) if len <= 2 => 2,
        Some((_, len)) if len <= 6 => 3,
        Some((_, len)) if len <= 15 => 4,
        Some(_) => 5,
    }
}

/// `research/JOURNAL.md` S1-P8's own remaining scope after S2-R20/S2-R22:
/// a "genuinely different 'more experts' signal," not another angle on
/// LZMA's matched-literal byte both of those slices already rejected. A
/// [`crate::match_model::MatchModel`] (its own hash table over the
/// reconstructed byte stream, independent of [`crate::lz`]'s own parse)
/// plus this expert's own mixing weight — but keyed by `WEIGHT_CONTEXTS`
/// *and* `match_len_bucket` together, so a long confirmed match and an
/// incidental one-off guess never share a weight, the length-conditioned
/// gating S2-R20's unconditional signal lacked.
#[derive(Debug, Clone)]
pub struct MatchModelExpertState {
    /// The underlying context-hash predictor. Not part of [`Literal`]'s own
    /// persisted state, same as [`PpmExpertState`]'s tables: constructed
    /// once per trial by whichever caller drives the paired measurement.
    engine: crate::match_model::MatchModel,
    /// One weight per ([`WEIGHT_CONTEXTS`] key, [`match_len_bucket`]
    /// bucket) pair.
    weight: Vec<f64>,
}

impl MatchModelExpertState {
    /// A fresh state: no history indexed yet, every weight starts at `1.0`
    /// (equally trusted), the same convention [`ColumnExpertState::new`]
    /// and [`PpmExpertState::new`] both use.
    #[must_use]
    pub fn new() -> Self {
        Self {
            engine: crate::match_model::MatchModel::new(),
            weight: vec![1.0; WEIGHT_CONTEXTS * MATCH_LEN_BUCKETS],
        }
    }

    /// Predicts the byte at `position` in `data`: [`crate::match_model::MatchModel::predict`]'s
    /// own doc names the causality contract (only `data[..position]` is
    /// ever consulted). Needs `data` and `position` directly, unlike every
    /// other expert in this module, because its own bank is not derived
    /// from [`Context`] alone — the same reason `research/JOURNAL.md`
    /// S2-R20's rejected matched-literal expert needed
    /// `codec::MatchByteExpertCostSink` to track state `Literal`'s own
    /// methods could not reach either.
    #[must_use]
    pub fn predict(&mut self, data: &[u8], position: usize) -> Option<(u8, u32)> {
        self.engine.predict(data, position)
    }

    /// This expert's own weight slot for `weight_index` (the same
    /// [`WEIGHT_CONTEXTS`] key [`banks`] selected for the six real experts)
    /// and `predicted`'s own [`match_len_bucket`].
    fn weight_slot(weight_index: usize, predicted: Option<(u8, u32)>) -> usize {
        weight_index * MATCH_LEN_BUCKETS + match_len_bucket(predicted)
    }
}

impl Default for MatchModelExpertState {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Literal {
    ...
    /// Eighth-expert-style counterpart of [`Self::mix_ppm`]: the same
    /// fixed-point blend with `state`'s own prediction folded in as one
    /// more, genuinely additive term (`research/JOURNAL.md` S1-P8). Unlike
    /// [`Self::mix_ppm`]'s per-symbol [`Ppm`] distribution, `predicted` is a
    /// single byte (or `None`): this expert's whole contribution is a
    /// spike at that one symbol, scaled by `predicted`'s own
    /// [`match_len_bucket`]-keyed weight, so a long confirmed match can
    /// earn real trust while a short or absent one earns none. `None`
    /// contributes nothing to any symbol, the same "silent, never a false
    /// floor" shape [`Self::mix_ppm`] already established for
    /// [`PpmExpertState`]'s zero-frequency case.
    fn mix_matchmodel(
        &self,
        bank_indices: &[usize; EXPERTS],
        weight_index: usize,
        predicted: Option<(u8, u32)>,
        state: &MatchModelExpertState,
    ) -> [u64; ALPHABET + 1] {
        let slot = MatchModelExpertState::weight_slot(weight_index, predicted);
        let w7 = state.weight[slot];
        let (weights6, weight_sum) = self.weights6_and_sum(weight_index, w7);
        let scale6 = self.scale6(bank_indices, &weights6, weight_sum);
        let predicted_symbol = predicted.map(|(byte, _)| usize::from(byte));

        let mut cum = [0u64; ALPHABET + 1];
        let mut acc = 0u64;
        for s in 0..ALPHABET {
            let mut mixed = self.six_expert_mixed(bank_indices, &scale6, s);
            if predicted_symbol == Some(s) {
                mixed += fixed_point_contribution_from_probability(w7, weight_sum, 1.0);
            }
            acc += (mixed >> 16) + 1;
            cum[s + 1] = acc;
        }
    ...
    /// Adapts `state`'s own weight slot ([`MatchModelExpertState::weight_slot`],
    /// keyed by both `weight_index` and `predicted`'s own
    /// [`match_len_bucket`]) toward `symbol`, the match-model counterpart of
    /// [`Self::update_ppm_expert`], then advances `state`'s own engine via
    /// [`crate::match_model::MatchModel::observe`] so its next
    /// [`MatchModelExpertState::predict`] call sees whether this prediction
    /// held.
    fn update_matchmodel_expert(
        &self,
        bank_indices: &[usize; EXPERTS],
        weight_index: usize,
        byte: u8,
        predicted: Option<(u8, u32)>,
        state: &mut MatchModelExpertState,
    ) {
        let symbol = usize::from(byte);
        let slot = MatchModelExpertState::weight_slot(weight_index, predicted);
        let w7 = state.weight[slot];
        let (weights6, weight_sum) = self.weights6_and_sum(weight_index, w7);
        let predicted_symbol = predicted.map(|(predicted_byte, _)| usize::from(predicted_byte));
        let estimate7 = if predicted_symbol == Some(symbol) {
            1.0
        } else {
            0.0
        };
        ...
        state.weight[slot] =
            self.adapt_seventh_weight(bank_indices, &weights6, weight_sum, symbol, w7, estimate7);

        state.engine.observe(predicted, byte);
    }

    /// `research/JOURNAL.md` S1-P8's before-wiring measurement, the same
    /// paired methodology [`Self::ideal_cost_bits_ppm_expert_pair`] and
    /// S1-P5's column expert both used: prices `byte` twice from the same
    /// pre-update six-expert state — once under the shipped mix
    /// ([`Self::ideal_cost_bits`] exactly, including its own `update` call,
    /// so the six real experts adapt on their one real trajectory
    /// regardless of this method ever running), once with `state`'s own
    /// match-model prediction blended in via [`Self::mix_matchmodel`],
    /// never substituted into any of the six real experts' own banks.
    /// `predicted` must come from [`MatchModelExpertState::predict`] called
    /// with this same byte's position, since this method (unlike every
    /// other expert pair here) cannot derive it from `context` alone.
    ///
    /// Returns `(baseline_bits, with_matchmodel_bits)`.
    #[must_use]
    #[allow(
        clippy::disallowed_methods,
        reason = "ideal-cost accounting never drives an Encoder or Decoder, so no bitstream depends on libm's last-ulp behavior here (ADR-0006, ADR-0024's determinism rule doesn't apply off the coding path)"
    )]
    pub fn ideal_cost_bits_matchmodel_expert_pair(
        &mut self,
        context: Context,
        byte: u8,
        predicted: Option<(u8, u32)>,
        ...
        let cum7 = self.mix_matchmodel(&bank_indices, weight_index, predicted, state);
        #[allow(
            clippy::cast_precision_loss,
            reason = "same bound as ideal_cost_bits: fixed-point sums stay well under 2^53"
        )]
        let with_matchmodel_probability =
            (cum7[symbol + 1] - cum7[symbol]) as f64 / cum7[ALPHABET] as f64;
        let with_matchmodel_bits = -with_matchmodel_probability.log2();

        self.update_matchmodel_expert(&bank_indices, weight_index, byte, predicted, state);

        let baseline_bits = self.ideal_cost_bits(context, byte);

        (baseline_bits, with_matchmodel_bits)
    }
}
...
mod tests {
    ...
    /// `research/JOURNAL.md` S1-P8: the six real experts must adapt on
    /// exactly the same trajectory whether or not this pairing method ever
    /// runs (`Self::ideal_cost_bits_matchmodel_expert_pair`'s own docs), the
    /// same claim `ppm_expert_pair_baseline_matches_plain_ideal_cost_bits`
    /// makes for `ideal_cost_bits_ppm_expert_pair`.
    #[test]
    fn matchmodel_expert_pair_baseline_matches_plain_ideal_cost_bits() {
        let mut paired = Literal::new();
        let mut plain = Literal::new();
        let mut state = MatchModelExpertState::new();
        let mut context = Context::default();
        for &b in b"the quick brown fox jumps over the lazy dog" {
            // No engine wired to real data here: every call predicts
            // nothing, which is exactly what this test needs (it only
            // checks the baseline side of the pair).
            let (baseline, _) =
                paired.ideal_cost_bits_matchmodel_expert_pair(context, b, None, &mut state);
            let expected = plain.ideal_cost_bits(context, b);
            assert!(
                (baseline - expected).abs() < 1e-9,
                "byte {b:?}: paired baseline {baseline} vs plain {expected}"
    ...
    /// `research/JOURNAL.md` S1-P8's own "silent, never a false floor"
    /// requirement: when no candidate exists, `mix_matchmodel`'s cum table
    /// must carry zero contribution from this expert at every symbol, not
    /// just at the one that would have been predicted.
    #[test]
    fn mix_matchmodel_contributes_nothing_when_no_match_is_found() {
        let model = Literal::new();
        let state = MatchModelExpertState::new();
        let context = Context::default();
        let (bank_indices, weight_index) = banks(context);

        let w7 = state.weight[MatchModelExpertState::weight_slot(weight_index, None)];
        let (weights6, weight_sum) = model.weights6_and_sum(weight_index, w7);
        let scale6 = model.scale6(&bank_indices, &weights6, weight_sum);

        let cum = model.mix_matchmodel(&bank_indices, weight_index, None, &state);
        for symbol in 0..ALPHABET {
            let expected_mixed = model.six_expert_mixed(&bank_indices, &scale6, symbol);
            let expected_entry = (expected_mixed >> 16) + 1;
            assert_eq!(
                cum[symbol + 1] - cum[symbol],
                expected_entry,
                "symbol {symbol}: match-model expert must contribute nothing when silent"
            );
        }
    }

    /// Mirrors `mix_ppm_first_entry_matches_the_six_expert_mix_plus_ppm_contribution`:
    /// when a candidate is found, its predicted symbol's own cum entry must
    /// carry exactly the six-expert mix plus this expert's own weighted
    /// spike, and no other symbol's entry may be affected.
    #[test]
    fn mix_matchmodel_adds_a_spike_at_the_predicted_symbol_only() {
        let model = Literal::new();
        let state = MatchModelExpertState::new();
        let context = Context::default();
        let (bank_indices, weight_index) = banks(context);
        let predicted = Some((b'z', 5u32));

        let w7 = state.weight[MatchModelExpertState::weight_slot(weight_index, predicted)];
        let (weights6, weight_sum) = model.weights6_and_sum(weight_index, w7);
        let scale6 = model.scale6(&bank_indices, &weights6, weight_sum);

        let cum = model.mix_matchmodel(&bank_indices, weight_index, predicted, &state);
        let predicted_symbol = usize::from(b'z');
        for symbol in 0..ALPHABET {
            let mut expected_mixed = model.six_expert_mixed(&bank_indices, &scale6, symbol);
            if symbol == predicted_symbol {
                expected_mixed += fixed_point_contribution_from_probability(w7, weight_sum, 1.0);
            }
            let expected_entry = (expected_mixed >> 16) + 1;
            assert_eq!(
                cum[symbol + 1] - cum[symbol],
                expected_entry,
                "symbol {symbol} did not match the expected six-expert-plus-spike contribution"
            );
        }
    }

    #[test]
    fn match_len_bucket_separates_absent_fresh_and_confirmed_matches() {
        assert_eq!(match_len_bucket(None), 0);
        assert_eq!(match_len_bucket(Some((b'a', 0))), 1);
        assert_eq!(match_len_bucket(Some((b'a', 2))), 2);
        assert_eq!(match_len_bucket(Some((b'a', 6))), 3);
        assert_eq!(match_len_bucket(Some((b'a', 15))), 4);
        assert_eq!(match_len_bucket(Some((b'a', 16))), 5);
        assert_eq!(match_len_bucket(Some((b'a', 1_000))), 5);
        // Different buckets must land in different weight slots for the
        // same WEIGHT_CONTEXTS key, so a long confirmed match's own weight
        // can adapt independently of a fresh guess's.
        let none_slot = MatchModelExpertState::weight_slot(0, None);
        let fresh_slot = MatchModelExpertState::weight_slot(0, Some((b'a', 0)));
        let confirmed_slot = MatchModelExpertState::weight_slot(0, Some((b'a', 20)));
        assert_ne!(none_slot, fresh_slot);
        assert_ne!(fresh_slot, confirmed_slot);
    }

    #[test]
    fn matchmodel_expert_pair_costs_stay_finite_and_positive() {
        let mut model = Literal::new();
        let mut state = MatchModelExpertState::new();
        let mut context = Context::default();
        let bytes = b"abcabcabcabcabcabcabcabcabcabc";
        for (position, &b) in bytes.iter().enumerate() {
            let predicted = state.predict(bytes, position);
            let (baseline, with_matchmodel) =
                model.ideal_cost_bits_matchmodel_expert_pair(context, b, predicted, &mut state);
            assert!(
                baseline.is_finite() && baseline > 0.0,
                "baseline={baseline}"
            );
            assert!(
                with_matchmodel.is_finite() && with_matchmodel > 0.0,
                "with_matchmodel={with_matchmodel}"
            );
            context = context.after_literal(b);
        }
    }
