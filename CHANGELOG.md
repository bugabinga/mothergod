...
- Train/sealed split plumbing, dataset-kind half (`research/JOURNAL.md`
  ...
  `DatasetKind::sealed_only` designates `GradientImage` and
  `SqliteLikeRecords` sealed-validation-only (`research/corpus/POLICY.md`,
  "held-out dataset kinds") — neither has a purpose-built filter in
  `src/filters.rs`, so they measure generalization undiluted by a filter
  tuned for exactly their shape. Regret scoring, the CI baseline gate,
  and progress-graph rendering remain.
