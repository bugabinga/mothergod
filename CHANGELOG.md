...
- `src/tans.rs`: `build_encode_table`, the tANS/FSE encode-table
  construction step (`research/JOURNAL.md` S1-P6, issue #447's next slice
  after `build_decode_table`), mirroring the decode side: an
  `EncodeTransform` per symbol plus the shared next-state table an
  encoder would index by register value. Cross-checked against
  `build_decode_table`'s own formulas over every reachable register
  value, not transcribed from a reference. Not yet wired to any `Method`
  or bitstream; no format or behavior change.
...
- `Model::new`/`Sse::new`/`Literal::new`'s fixed-size tables and the
  ...
  `codec::undo_filter` on the buffered real decode path; the panicking
  originals stay for the encoder and every test, where an allocator
  failure this small is already fatal to the whole process.
  `tests/torture.rs`'s allocation-failure sweep is fully green now (73
  allocator calls across the golden and adversarial corpus, was
  previously finding 66 aborts of 123 calls), closing issue #453.
  `filters::delta::Undo`/`filters::bcj::Undo`, the streaming decode
  path's own per-byte undo state (reachable only via
  `decompress_to_writer`, not swept by `tests/torture.rs`), got the same
  `try_new` treatment; so did `decompress_to_writer`'s internal
  `std::io::BufWriter`, whose own buffer allocation was unconditionally
  infallible regardless of input. A deeper, separate finding survives:
  every error `decompress_to_writer` returns still allocates via
  `std::io::Error::other`'s inherent (and infallible on stable Rust)
