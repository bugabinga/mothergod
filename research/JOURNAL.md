...
- S1-P8 | LEAD | GLN-style predictors / more experts (2026 AIT Challenge
  ...
  **-0.001661 b/B** (mean; `json_records` -0.022140 and `base64_wrapped`
  ...
  suppression) have now each traded off between `gradient_image`'s tax and
  real train-side signal without clearing both bars at once — the same
  "unclear, no further named branch" state S1-P2/S1-P3 both reached. What
  is left is the from-scratch GLN gating architecture this lead's own name
  still names (declined near S2-A99, again after S2-R23, and a third time
  here, under a single session's time budget every time), or accepting
  this lead's ceiling absent one.
