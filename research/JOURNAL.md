...
- S2-A77 | ACCEPTED | S1-P6's own first slice (issue #447), the
  ...
  (`docs/benchmarks/canterbury.md`, real bitstreams, this sandbox):
  `xargs.1` (the file S2-A67 first flagged under the floor) measured
  0.596–0.721 MB/s decode across regeneration noise on this machine,
  still under the 1 MB/s floor — this slice narrows the gap, it does not
  close it. | Mechanism: full argument lives in `Literal::mix`'s own
