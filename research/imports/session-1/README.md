...
| `autoresearch2.py` | Research-loop harness + Python codec v3 | The later of two uploaded revisions (adds `algn` alignment expert, `dpcand`, `selo1`/`seltrial` filter-selection arms; the earlier revision was a strict subset and was not kept). **Incomplete**: line 2 `exec`s a base file `autoresearch.py` (models `M`, `FILTERS`, `CTX`, `bkt`, …) that was NOT uploaded — the harness does not run without it. Either the operator supplies it, or the port reconstructs the missing primitives (they are small and inferable from usage). |
