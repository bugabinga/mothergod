...
- PR bodies and PR comments end with the footer, typed: a blank line, `---`,
  then `_Generated by [Claude Code](https://claude.ai/code)_`. Issues and
  issue comments never carry a typed one: gh-comment appends the role
  footer, so typing it there doubles it (every heartbeat post did, until
  2026-09-12).
